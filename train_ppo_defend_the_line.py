...
from defend_the_line.pipeline_ppo_defend_the_line import DefendLinePPOPipeline
...
def train(
    ...
    ml_pipeline = DefendLinePPOPipeline(
        SCENARIO_ID, DEFEND_LINE_DISCRETE_ACTIONS, DefendLineDoomEnv
    )
    run_name = f"ppo_{SCENARIO_ID}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Train PPO on {SCENARIO_ID}"
    )
    parser.add_argument("--timesteps", type=int, default=5_000_000,
                        help="Total environment steps (default: 5M)")
